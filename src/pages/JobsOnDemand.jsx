              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 flex items-center gap-2 flex-wrap">
                    ...
                    <span>•</span>
                    <span className="flex items-center gap-1">{job.locationCity ? <><FiMapPin className="w-3 h-3"/> {job.locationCity}</> : (job.locationType === 'REMOTE' ? 'Remoto' : job.locationType)}</span>
                    <span>•</span>
            ...
            <div className="p-5">
              ...
              <div className="mt-5 flex flex-col items-start gap-2">
                <div className="w-full sm:w-auto flex flex-wrap items-center gap-3">
                    ...
                    : job.hasApplied ? (
                        confirmingCancelId === job.id ? (
                            <div className="p-3 bg-red-50 dark:bg-red-900/10 rounded-lg border border-red-100 flex gap-2 items-center animate-in fade-in">
                                ...
                                <button onClick={() => setConfirmingCancelId(null)} className="text-xs px-2 py-1 bg-white border rounded">Não</button>
                                <button onClick={() => handleConfirmCancel(job.id)} disabled={actionLoading === job.id} className="text-xs px-2 py-1 bg-red-600 text-white rounded">Sim</button>
                            </div>
                        ) : (
                            <div className="flex items-center gap-3">
                                ...
                                <button onClick={() => handleCancelClick(job.id)} disabled={actionLoading === job.id} className="text-red-500 hover:text-red-600 text-xs hover:underline disabled:opacity-50 transition-colors">
                                    Cancelar
                                </button>
                            </div>
                        )
                    ...
                    : (
                        <button onClick={() => handleApply(job.id)} disabled={actionLoading === job.id} className="px-6 py-2 bg-sky-600 hover:bg-sky-700 text-white rounded-lg text-sm font-medium transition-colors disabled:opacity-50 w-full sm:w-auto">
